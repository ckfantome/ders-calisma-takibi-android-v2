...
object PrivacyConsent {
    ...
    val TEXT_TR = """
        ...
        - TAM GİZLİ MOD (yalnızca Uygulama Kilidi ekranından AYRICA ve AÇIKÇA etkinleştirilirse - varsayılan KAPALIDIR): Uygulama Yöneticisi bu özelliği açarsa, uygulama simgesi telefonun başlatıcısından/ana ekranından ve Son Kullanılanlar listesinden kaldırılır; öğrenci uygulamayı normal yollarla göremez/açamaz. Bu özellik YENİ BİR VERİ TOPLAMA YAPMAZ, sadece simgenin görünürlüğünü etkiler. Yönetici, Arkaplan Servisi'nin devam eden takip bildirimine dokunarak uygulamayı yine de açabilir; bu bildirim, Arkaplan Servisi çalışırken durum çubuğunda görünür kalır.
    ...
    val TEXT_EN = """
        ...
        - FULLY HIDDEN MODE (only if SEPARATELY and EXPLICITLY enabled from the App Lock screen - OFF by default): if the App Administrator turns this on, the app icon is removed from the phone's launcher/home screen and from the Recents (app switcher) list; the student cannot see or open the app through normal means. This feature does NOT add any new data collection, it only affects the visibility of the icon. The Administrator can still open the app by tapping the ongoing tracking notification from the Background Service, which remains visible in the status bar while that service is running.
